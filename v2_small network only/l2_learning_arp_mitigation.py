...
def handle_spoof(mac=None):
    ...
    msg = of.ofp_flow_mod(command=of.OFPFC_ADD, idle_timeout=60, 
        hard_timeout=60, #60 seconds 
    ...
    if packet.type == packet.ARP_TYPE:
		if packet.payload.opcode == pkt.arp.REQUEST :
			...
			# Check if IP exists in the hash table
			if src_mac_eth != src_mac_arp :
				...
				# Check if source IP and source MAC are matched 
				if EthAddr(hosts[str(src_ip_arp)]) != src_mac_arp:
					drop()
				        handle_spoof(src_mac_eth)
					return
				else:
					# Check if the destination host is already in the network
